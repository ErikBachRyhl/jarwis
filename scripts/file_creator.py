...
def generate_files(response):
    topic = next(iter(response))
    topic_tag = next(iter(response)).replace(" ", "")

    def generate_md_files(name, content, immediate_parent, depth):
       #time.sleep(.25)
    ...
    try:
        for k, v in response.items():
            generate_md_files(k, v, "", 0)  # initial depth is 0
